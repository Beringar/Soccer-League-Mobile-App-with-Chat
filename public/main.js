...
function scoresList() {
	...
	for (var i = 0; i < matchesArray.length; i++) {
		...
		if (match_datetime < today) {
			...
			match_score_item_Html = "<div class='score_item centered_text' id='" + matchesArray[i].match_id + "'><p class='score_team_name'>" + teamsArray[matchesArray[i].local_team_id].team_name + " vs. " + teamsArray[matchesArray[i].visitor_team_id].team_name + "</p><p class='score_result'>" + matchesArray[i].local_team_score + " - " + matchesArray[i].visitor_team_score + "</p><p class='score_date'>Date: " + output_date + "</p></div><hr class='hr_scores'>";
